...
public class Connection implements Closeable {
    ...
    //
    // public boolean ping() {
    //     Header heartBeatHeader = new Header(Const.MAGIC, Const.VERSION_1);
    //     heartBeatHeader.setExtraInfo(Const.HEART_EXTRA_INFO);
    //     Message message = new Message(heartBeatHeader, null);
    //     NettyResponseFuture<Response> request = request(message, Const.DEFAULT_TIMEOUT);
    //     try {
    //         Promise<Response> await = request.getPromise().await();
    //         return await.get().getCode() == Const.HEARTBEAT_CODE;
    //     } catch (Exception e) {
    //         return false;
    //     }
    // }
